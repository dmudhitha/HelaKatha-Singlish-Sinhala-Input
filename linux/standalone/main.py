...
# Add core and local directories to Python path
...
# ==========================================================
# Native X11 Direct Unicode Keystroke Injector (0% Clipboard)
# ==========================================================
import ctypes
from ctypes import c_int, c_uint, c_ulong, c_void_p, POINTER

class X11DirectInjector:
    def __init__(self):
        self.available = False
        self.display = None
        self.keycode = 250 # Safe high keycode range in X11
        try:
            ...
            self.x11.XChangeKeyboardMapping.argtypes = [
                c_void_p, c_int, c_int, POINTER(c_ulong), c_int
            ]
            self.xtst.XTestFakeKeyEvent.argtypes = [
                c_void_p, c_uint, c_int, c_ulong
            ]
            ...
            print(f"X11 direct injector not available: {e}")
            self.available = False
    ...
    def char_to_keysym(self, ch):
        ...
        if cp < 0x80:
            return cp
        if cp >= 0x0100:
            return 0x01000000 | cp # Standard X11 Unicode Keysym mapping
        return cp
    ...
    def type_string(self, text):
        ...
        try:
            for ch in text:
                keysym = self.char_to_keysym(ch)
                keysym_arr = (c_ulong * 1)(keysym)
                self.x11.XChangeKeyboardMapping(self.display, self.keycode, 1, keysym_arr, 1)
                self.x11.XSync(self.display, 0)
                self.xtst.XTestFakeKeyEvent(self.display, self.keycode, 1, 0) # KeyDown
                self.xtst.XTestFakeKeyEvent(self.display, self.keycode, 0, 0) # KeyUp
                self.x11.XFlush(self.display)
                time.sleep(0.002)
            return True
        except Exception as err:
            print(f"Direct X11 typing failed: {err}")
            return False
...
class SinglishInputController(QObject):
    ...
    def __init__(self):
        ...
        # 2. Virtual Key Injector & Native X11 Unicode Keystroke Injector (Zero Clipboard)
        self.keyboard_controller = KeyboardController()
        self.x11_injector = X11DirectInjector()
    ...
    def on_press(self, key):
        # Ignore synthetic Backspaces emitted during injection
        if self.is_injecting:
            if key == Key.backspace or getattr(key, 'char', None) is None:
                return
    ...
    def on_click(self, x, y, button, pressed):
        # Click outside hides suggestion popup
    ...
    def handle_replace_text(self, delete_count, selected_word, trigger_char):
        ...
        # 1. Delete typed English text and trigger key cleanly
        ...
        # 2. Inject Sinhala text block (Renders 100% of combining marks, ZWJ, and vowel signs)
        try:
            ...
            # Save whatever you previously copied
            clipboard = QApplication.clipboard()
            orig_clip = clipboard.text(QClipboard.Mode.Clipboard)
            
            # Set the Sinhala word for atomic injection
            clipboard.setText(text_to_inject, QClipboard.Mode.Clipboard)
            if clipboard.supportsSelection():
                clipboard.setText(text_to_inject, QClipboard.Mode.Selection)
            
            # Send Ctrl+V using xdotool (Bypasses browser synthetic event sandboxes in Chrome, Brave, Firefox)
            ...
            # Let the application receive and complete the paste event
            loop = QEventLoop()
            QTimer.singleShot(150, loop.quit)
            loop.exec()
            
            # IMMEDIATELY RESTORE YOUR REAL CLIPBOARD CONTENT!
            # Pressing Ctrl+V later will NEVER paste this Sinhala word!
            if orig_clip:
                clipboard.setText(orig_clip, QClipboard.Mode.Clipboard)
                if clipboard.supportsSelection():
                    clipboard.setText(orig_clip, QClipboard.Mode.Selection)
            else:
                clipboard.clear()
        except Exception as inject_err:
            print(f"Injection error: {inject_err}")
